# location3

国土数値情報 N05-24（鉄道時系列）を使い、1950〜2024年の鉄道路線と駅を地図でたどる Web アプリケーション「鉄道の時間地図」です。スマホでは地図の下に年代操作をまとめ、検索と表示設定を折りたためます。

## 使い方

```bash
corepack pnpm install
npm run build
go run ./cmd/webapp
```

- `-addr` でリッスンアドレスを変更できます（既定は `:8080`）。
- `-utf8-dir` に UTF-8 版データセットのディレクトリ（例: `N05-24_GML/UTF-8`）を渡せます。
- `-static-dir` で静的ファイルの配置ディレクトリを差し替えられます（既定は `.generated/assets`）。画面を編集したら `npm run build` で再生成します。

起動後、ブラウザで `http://localhost:8080/` を開くとマップが表示されます。

### 地図の操作

- 初期表示はデータの最新年である **2024年**。スライダー、前後ボタン、年の直接入力、年代ショートカットで切り替えられます。
- 「全期間を見る」で全年代のデータを重ねて表示します。年代を変えても地図の位置・ズームを保ちます。
- 駅名・路線名を検索し、候補を選ぶと地図上へ移動します。検索対象は選択中の年代に存在するデータです。
- スマホでは下部に年・スライダー・1年/10年送りをまとめ、地図を広く表示します。「駅・路線を探す」で検索パネル、「表示・保存」で表示切替・年代ショートカット・オフライン保存を開けます。
- 検索候補を選ぶとパネルを閉じて地図へ移動します。入力中のパネルは画面の表示領域に合わせて高さを調整し、「地図に戻る」は下部に固定します。
- 現在地ボタンは押したときだけブラウザに位置情報を要求します。HTTPS または localhost で利用できます。
- 「共有」で年代・地図位置・表示レイヤーを含むリンクを共有できます。端末が共有機能に対応していない場合はリンクをコピーできます。
- 地図の四隅アイコンで日本全体を表示します。キーボードの矢印キーでも地図や年スライダーを操作できます。

データの判定は**年単位**です。同じ年の中の開業・廃止日は区別しません。新しい共有URLは `?year=1980&lat=35.68&lng=139.76&zoom=12` の形式で、従来の `?date=1980-01-01` も読み込めます。全期間は `?year=all` です。年の選択範囲はデータの対象期間である1950〜2024年です。

### オフラインで使う

1. 通信できる場所で、保存したい地域を地図に表示します。
2. スマホでは「表示・保存」を開き、「この周辺を保存…」を押します。
3. 容量を確認して名前を付け、「保存する」を押します。完了まで画面を開いたままにしてください。
4. 保存後は圏外でもページを開き直し、背景地図・全年代の切替・駅や路線の検索を使えます。「保存した地図だけ使う」で保存範囲を確認できます。保存した地域から地図へ移動・削除できます。

初回は全年代の鉄道データ約29MiBと日本全体の概略地図約13MiB、選択範囲の詳細地図、画面のファイルを保存します。地域は周辺を含む区画単位で保存し、共通ファイルは再利用します。1地域の上限は200MiBで、広すぎる場合は拡大してから保存します。詳細地図はズーム14までのデータを拡大表示します。保存範囲外は概略地図と鉄道データのみ利用できます。

保存はその端末・ブラウザに限られ、別端末には同期しません。ブラウザのデータ削除や容量不足による自動削除で消える場合があります。HTTPSまたはlocalhostと、Service Worker / CacheStorage対応ブラウザが必要です。ホーム画面に追加すると圏外でも開きやすくなります。現在地は端末で位置を取得できれば利用できます。

背景地図はProtomapsの地域別PMTilesを当サイトから配信します。MapLibre GL JS、PMTiles読み込みコード、画面を同じ配信元に置き、ラベルは端末のフォントで描画します。外部CDNやOpenStreetMap標準タイルサーバーは利用しません。背景は現在の地図で、年代を変えても変化しません。

通常閲覧では256KiB単位の静的ファイルから必要な部分だけ取得します。Cloudflare Static AssetsのRange非対応に依存せず、Workerで地図を展開・解析する処理もありません。明示的な地域保存時は完全なアーカイブを取得し、容量・SHA-256を検証してから利用可能にします。

## データソースと権利表記

- 鉄道路線・駅データは、国土交通省 国土数値情報（鉄道時系列）N05-24 を使用しています。出典: [https://nlftp.mlit.go.jp/ksj/gml/datalist/KsjTmplt-N05-2024.html](https://nlftp.mlit.go.jp/ksj/gml/datalist/KsjTmplt-N05-2024.html)
- 利用にあたっては必ず [国土数値情報利用約款](https://nlftp.mlit.go.jp/ksj/other/data_license.html) を遵守してください。本アプリケーションでは当該約款の「出典を明記すること」等の条件に従い、表示画面および README に利用データの出典を記載しています。
- 国土交通省が提供する原データに基づいていますが、アプリケーションの表示内容は独自に加工したものであり、国土交通省が保証するものではありません。
- 背景地図は [Protomaps](https://protomaps.com)、[© OpenStreetMap contributors](https://www.openstreetmap.org/copyright)（ODbL）、Natural Earth。配布元・生成手順・固定版の詳細は [basemaps/README.md](basemaps/README.md) に記録しています。

## 開発

Go 1.25.1以降、JavaScriptのテストにはNode.js 22.18以降（TypeScriptの型消去に対応した版）を使用します。

```bash
go test ./...
npm test
```

フロントエンドはHTML・CSS・ES Modulesです。`npm run build` はデータ生成、地域別地図の検証、ライブラリ同梱、オフライン画面のキャッシュ一覧生成を行います。Workersへの公開時も自動実行します。初回ビルドでは不足している背景地図を本番の同一ハッシュURLから取得するため、約1.64GiBの通信が発生します。

| ファイル | 役割 |
| --- | --- |
| `web/static/index.html`, `styles.css` | レスポンシブな画面構成・スタイル |
| `web/static/app.mjs` | UIイベント、読み込み状態、検索・共有の操作 |
| `web/static/mobile-panel.mjs` | スマホの検索・設定パネル、キーボード表示時の高さ調整 |
| `web/static/map.mjs`, `basemap.mjs` | MapLibre描画・PMTilesの地域選択・地図の操作 |
| `web/static/offline-store.mjs`, `offline-ui.mjs` | 保存計画、検証付きダウンロード、共有ファイルの削除、保存画面 |
| `web/static/service-worker.mjs` | 画面とライブラリのオフライン起動・版ごとの一括更新 |
| `web/static/railway-filter.mjs` | ビルドと端末で共通の年代絞り込み |
| `web/static/state.mjs` | URLの読み書きと入力値の検証 |
| `web/static/data.mjs` | API取得・キャンセル・直近2期間のキャッシュ・検索 |
| `internal/server/` | ローカルデータを配信するGoサーバー |
| `worker/` | 生成済みGeoJSONをストリーム配信するCloudflare Worker |
| `scripts/build.mjs`, `prepare-data.mjs`, `prepare-basemap.mjs` | データ・背景地図・画面を公開前に生成 |
| `tests/`, `internal/server/testdata/` | フロントエンドのロジックテストと共通API fixture |

両バックエンドは `/api/railroads` と `/api/stations` のGET/HEADに対応します。`date=YYYY-MM-DD` を指定すると年単位で絞り込み、未指定なら全件を返します。存在しない日付は400です。駅は当該年の路線名でも絞り込みます。Workersでは公開前に全期間・各年のGeoJSONを生成し、リクエスト時は小さな索引から該当ファイルを選んでストリーム配信します。全座標のJSON解析・再シリアライズは行いません。

APIはETagと5分間のHTTPキャッシュに対応します。再検証時に `If-None-Match` が一致すれば、GET/HEADとも本文なしの304を返します。内容が変われば新しいETagと本文を200で返します。WorkersではStatic Assetsの照合結果をそのまま利用し、検証のためにGeoJSONを読み込むこともありません。ブラウザは保存済みの本文を再利用するため、フロントエンド側で304を処理する必要はありません。

ブラウザでは年代変更時に古いリクエストを中断し、MapLibreのWeb Workerで地図を描画します。保存済みデータがあれば端末で絞り込み、API通信を省きます。保存データが使えない場合はオンライン取得へ戻ります。取得失敗時は地図内に再試行ボタンを表示します。

Pull Request では `go test` の結果や使用した追加ツールがあれば明記してください。

## Cloudflare Workers へのデプロイ

Workers Static Assetsにフロントエンドと生成済みGeoJSONをまとめて公開します。Worker実行時に全国データを解析するとCPU時間・メモリ制限に達するため、重い加工はローカルのビルド時に行います。

1. Node.js 22.18以降とWranglerを用意し、`wrangler login` で認証します。
2. UTF-8の原本2ファイルを `N05-24_GML/UTF-8/` に配置します。元ZIPやShift-JIS版は変更しません。
3. `npm run deploy`（または `wrangler deploy` / `make deploy-worker`）を実行します。`wrangler.toml` の `[build]` により事前生成が自動で動き、`.generated/assets` を公開します。
4. 公開後、`/api/railroads?date=1966-01-01` と `/api/stations?date=1966-01-01` が200になることを確認します。N05-24原本ではそれぞれ1,014件・13,210件です。

生成だけを行う場合：

```bash
npm run build
# 鉄道データだけを生成する場合（地図ライブラリ等は含みません）
npm run build:data
# 入力を指定する場合
node scripts/prepare-data.mjs --input-dir N05-24_GML/UTF-8 --output-dir .generated/assets
```

生成処理は開始年・終了年の境界ごとに結果を計算するため、APIの0001〜9999年の指定にも従来と同じ結果を返します。同一内容はハッシュで重複排除します。生成物は大きいためGitには含めず、原本から再現します。1ファイルがStatic Assetsの25MiB上限を超えた場合は公開前にエラーにします。原本のままでも、空白を除去した全期間の路線データは約21.8MiBに収まります。

索引・GeoJSON・画面は同じWorkerバージョンのアセットとしてまとめて公開されます。既存のR2バケットに保存した原本は削除しませんが、配信時には使用しません。原本がGit管理外なので、CIで公開する場合も生成前に原本を配置してください。

カスタムドメインは `wrangler.toml` の `[[routes]]` で設定します。

### よく使う Wrangler コマンド

`Makefile` にラッパーターゲットを用意しています。

```bash
# Worker のローカルプレビュー
make worker-dev

# 本番デプロイ
make deploy-worker
```

Workers 上での利用時も、国土数値情報利用約款と OpenStreetMap のクレジット表記を忘れずに行ってください。
